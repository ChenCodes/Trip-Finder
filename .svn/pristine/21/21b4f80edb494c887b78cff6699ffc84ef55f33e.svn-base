...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = counter; i < segment.size(); i++) {
            if (i < segment.size() - 1) {
                Integer firstVertex = segment.get(i);
                Integer secondVertex = segment.get(i + 1);
                Road firstRoad = _map.getLabel(firstVertex, secondVertex);
                if (secondRoad == null
                    || !firstRoad.toString().equals(secondRoad.toString())
                    || !firstRoad.direction().equals(secondRoad.direction()))  {
                    if (secondRoad != null) {
                        System.out.print(seq);
                        System.out.print(". ");
                        System.out.print("Take ");
                        System.out.print(secondRoad.toString());
                        System.out.print(" ");
                        System.out.print(secondRoad.direction().fullName());
                        System.out.print(" for ");
                        System.out.print((Math.round(milesTraveled
                            * 10.0) / 10.0));
                ...
                milesTraveled += firstRoad.length();
            } else {
                System.out.print(seq);
                System.out.print(". ");
                System.out.print("Take ");
                ...
                System.out.print(secondRoad.direction().fullName());
                System.out.print(" for ");
                System.out.print((Math.round(milesTraveled * 10.0) / 10.0));
                System.out.print(" miles");
                System.out.print(" to ");
                System.out.print(_finalLocation2.toString());
                System.out.print(".");
                System.out.println();
                seq += 1;
            }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        TripPlan(int start, int dest) {
            ...
            _finalLocation2 = _map.getLabel(dest);
        ...
        protected double estimatedDistance(int v) {
            if (_map.contains(v)) {
                Location starting = _map.getLabel(v);
                return starting.dist(_finalLocation);
            }
            return 0.0;
        }

        /** Location of the destination. */
        protected final Location _finalLocation;
