...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int current : _depends.successors(this.getVertex())) {
            if (_depends.getLabel(current).isUnfinished()) {
    ...
    private boolean outOfDate() {
        if (_time == null) {
            ...
            for (int current : _depends.successors(this.getVertex())) {
                if (this.getTime() < _depends.getLabel(current).getTime()) {
                    return true;
                }
            }
        }
